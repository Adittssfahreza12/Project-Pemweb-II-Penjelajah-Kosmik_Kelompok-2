...
-- ============================================================
-- TABEL 2 : sesi_login  (opsional – untuk fitur "Ingat Saya")
-- Menyimpan token sesi agar pengguna tidak perlu login ulang
-- ============================================================
CREATE TABLE IF NOT EXISTS `sesi_login` (
  `id`         INT UNSIGNED  NOT NULL AUTO_INCREMENT,
  `user_id`    INT UNSIGNED  NOT NULL,
  `token`      VARCHAR(255)  NOT NULL UNIQUE COMMENT 'Token acak yang disimpan di cookie',
  `expired_at` DATETIME      NOT NULL COMMENT 'Waktu kadaluarsa token',
  `created_at` DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_sesi_user` (`user_id`),
  CONSTRAINT `fk_sesi_user`
    FOREIGN KEY (`user_id`) REFERENCES `users`(`id`)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Tabel sesi login – dipakai fitur Ingat Saya';
...
-- ============================================================
-- TABEL 4 : detail_jawaban_kuis  (opsional – breakdown per soal)
...
CREATE TABLE IF NOT EXISTS `detail_jawaban_kuis` (
  ...
  `jawaban_pilihan` TINYINT UNSIGNED NOT NULL COMMENT 'Indeks jawaban yang dipilih user (0–3)',
