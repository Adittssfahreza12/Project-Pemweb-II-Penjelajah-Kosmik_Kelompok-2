...
async function saveCalculatorHistory(payload) {
    const user = getActiveUser();
    if (!user || !user.id) return;

    try {
        await postJSON('kalkulator_api.php', {
            action: 'save',
            user_id: user.id,
            ...payload
        });
...
async function syncPlanetFavorite(action, payload) {
    const user = getActiveUser();
    if (!user || !user.id) return;

    try {
        await postJSON('favorit_api.php', {
            action,
            user_id: user.id,
            ...payload
        });
...
function calculate() {
    ...
    planets.forEach((p) => {
        ...
        planetResults.push({
            nama_planet: p.name,
            emoji_planet: p.emoji,
            gravitasi: p.gravity,
            berat_di_planet: parseFloat(weightOnPlanet)
        });
    ...
    void saveCalculatorHistory({
        berat_badan_kg: userWeight,
        tinggi_badan_cm: userHeight,
        jenis_kelamin: gender,
        bmi_bumi: parseFloat(bmi),
        kategori_bmi: bmiCat.cat,
        planet_results: planetResults
    });
}
...
function toggleFavFromCalc(e, name, emoji, type, weightOnPlanet, weightOnEarth) {
    ...
    if (idx >= 0) {
        ...
        void syncPlanetFavorite('remove', { kategori: 'planet', nama: name });
    } else {
        favs.push({
            id: Date.now(),
            name, emoji, type,
        ...
        void syncPlanetFavorite('add', {
            kategori: 'planet',
            nama: name,
            emoji,
            tipe: type,
            berat_di_planet: parseFloat(weightOnPlanet),
            berat_di_bumi: parseFloat(weightOnEarth)
        });
    }
